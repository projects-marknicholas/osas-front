...
import { Plus, Trash2, Search, AlertCircle, Edit, Upload } from "lucide-react";
...
const ManageScholarshipForms = () => {
  ...
  const fetchForms = async () => {
    try {
      ...
      console.error("Failed to fetch scholarship forms:", err);
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to load scholarship forms. Please try again.",
  ...
  const handleAddForm = async (e) => {
    e.preventDefault();
    if (!newFormName.trim() || !newFile) {
      Swal.fire({
        icon: "warning",
        title: "Missing Fields",
        text: "Please enter a form name and select a file.",
        confirmButtonColor: "#2d6179",
      });
      return;
    }
    ...
    try {
      const res = await createScholarshipForm({
        scholarship_form_name: newFormName,
        file: newFile,
      });
      if (res.success) {
        Swal.fire({
          icon: "success",
          title: "Added!",
          text: `Scholarship Form "${newFormName}" has been added.`,
        ...
        throw new Error(res.error || "Failed to add scholarship form");
      }
    } catch (err) {
      Swal.fire({
        icon: "error",
        ...
        try {
          ...
          if (res.success) {
            Swal.fire({
              icon: "success",
              title: "Deleted!",
              text: `Scholarship Form "${form_name}" has been deleted.`,
              confirmButtonColor: "#2bb9c5",
            });
            fetchForms();
          } else {
            throw new Error(res.error || "Failed to delete scholarship form");
          }
        } catch (err) {
          Swal.fire({
            icon: "error",
  ...
  const handleEditForm = async (form) => {
    const { value: formValues } = await Swal.fire({
      title: "Edit Scholarship Form",
      html: `
        <input id="form-name" class="swal2-input" value="${form.scholarship_form_name}" placeholder="Form name" />
        <input id="form-file" type="file" class="swal2-file" />
      `,
      focusConfirm: false,
      preConfirm: () => {
        const name = document.getElementById("form-name").value;
        const file = document.getElementById("form-file").files[0] || null;
        if (!name.trim()) {
          Swal.showValidationMessage("Form name cannot be empty");
        }
        return { name, file };
      },
      showCancelButton: true,
    ...
    if (formValues) {
      try {
        const res = await editScholarshipForm({
          scholarship_form_id: form.scholarship_form_id,
          scholarship_form_name: formValues.name.trim(),
        ...
        if (res.success) {
          ...
          throw new Error(res.error || "Failed to update scholarship form");
        }
      } catch (err) {
        Swal.fire({
          icon: "error",
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Upload, edit, and manage scholarship form templates
          </p>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          >
            ...
            <span>Add Scholarship Form</span>
          </button>
        </div>
      </div>
      ...
      {showAddForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          ...
          <form onSubmit={handleAddForm} className="flex items-center space-x-4">
            <div className="flex-1 flex flex-col space-y-3">
              ...
              />
              <div
                onDrop={(e) => handleDrop(e, setNewFile)}
              ...
              >
                {newFile ? (
                  <p className="text-gray-700">{newFile.name}</p>
                ) : (
                ...
                forms.map((form) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      <a
                        href={`${import.meta.env.VITE_FOLDER_URL}/scholarship_forms/${form.scholarship_form}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-[#2d6179] hover:underline"
                      >
                        View File
                      </a>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap flex space-x-2">
                      ...
                      <button
                        onClick={() =>
                          handleDeleteForm(form.scholarship_form_id, form.scholarship_form_name)
                        }
                ...
                <tr>
                  <td colSpan="3" className="text-center py-12">
                    ...
                    <p className="text-gray-600">
                      {searchTerm
                        ? `No scholarship forms match "${searchTerm}"`
                        : "No scholarship forms have been uploaded yet."}
                    </p>
                  </td>
                </tr>
              )}
